    ...
    <div class="wrapper">
      <ul class="paper">
        <li class="item" v-for="(item, index) in pagination" :key="index"  @click="toExam(item)">
          ...
          <div class="info">
            <i class="el-icon-loading"></i><span>{{ item.paperDate }}</span> <i
              class="iconfont icon-icon-time"></i
...
export default {
  ...
  methods: {
    ...
    getExamInfo() {
      ...
      this.axios
          .post('/wsyu/paper/queryPaperPage.htm', params)
          .then(
              function (response) {
    ...
    sendForm(val) {
      if (val) {
        ...
        this.axios
            .post('/wsyu/paper/successPaper.htm', params)
            .then(
                function (response) {
    ...
    toExam(item) {
      if (item.status === 0) {
        ...
        this.axios
            .post('/wsyu/paper/addPaperByStudent.htm', params)
            .then(
                function (response) {
                  if (response.data.code == '0000') {
