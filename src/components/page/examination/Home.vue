<template>
  <div>
    ...
    <el-row :gutter="20">
      <el-col
        :span="2"
        style="margin-left: 80%; margin-bottom: 5px; margin-top: -5px"
      >
        <el-button @click="collBackPaperPage" type="primary"
          >返回试卷</el-button
        >
      </el-col>
      <el-col :span="2">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            <el-button size="mini" @click="queryPaper(scope.row)"
              >查看试卷</el-button
            >
            <el-button
              size="mini"
              ...
              >删除</el-button
            >
            <el-button size="mini" @click="exportPaper(scope.row.paperId)"
              >试卷导出</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    updateTitle(dataList) {
      ...
      this.axios
        .post("/wsyu/title/updateTitleByList.htm", params)
        .then(
          function (response) {
    ...
    onSubmit() {
      ...
      Loading.service();
      this.axios
        .post("/wsyu/title/audioExam.htm", params)
        .then(
          function (response) {
            if (response.data.code == "0000") {
    ...
    exportPaper(paperId) {
      ...
      form.action = "/wsyu/paper/paperExport.htm";
      form.submit();
    ...
    handleDelete(row) {
      ...
      this.axios
        .post("/wsyu/paper/deletePaper.htm", params)
        .then(
          function (response) {
            if (response.data.code == "0000") {
    ...
    queryPaperPage() {
      ...
      this.axios
        .post("/wsyu/title/queryPaperPage.htm", params)
        .then(
          function (response) {
    ...
    queryPaper(row) {
      ...
      this.axios
        .post("/wsyu/title/queryPaper.htm", params)
        .then(
          function (response) {
            if (response.data.code == "0000") {
              this.formShow = false;
              ...
              this.examShow = true;
              this.dataList.push({
                list: this.changeArray(
                  response.data.result.oneList1,
                  1,
                  "type"
                ),
              });
