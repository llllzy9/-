<template>
  <div>
    ...
    <el-row :gutter="10" style="margin-bottom:10px;margin-left:900px">
      <el-col :span="10">
        <el-input v-model="titleName" placeholder="请输入题目"
    ...
    <el-table :data="tableData" height="720" stripe>
      <el-table-column type="index" width="50"> </el-table-column>
      <el-table-column prop="titleName" label="题目" width="180">
      </el-table-column>
      <el-table-column prop="subjectName" label="科目"> </el-table-column>
      <el-table-column prop="className" label="班级"> </el-table-column>
      <el-table-column prop="titleFraction" label="分值"> </el-table-column>
      <el-table-column
        prop="titleStatus"
        label="题目类型"
      ...
      <el-table-column prop="titleType" label="难度系数"> </el-table-column>
      <el-table-column prop="userName" label="出题人"> </el-table-column>
      <el-table-column prop="titleAnswer" label="答案"> </el-table-column>
      <el-table-column prop="createTime" label="时间" width="180">
      </el-table-column>
      <el-table-column fixed="right" label="操作" width="100">
        <template slot-scope="scope">
          <el-button
            @click="handleClick(scope.row.titleId)"
            type="text"
            size="small"
            >编辑</el-button
          >
          <el-button
            type="text"
            size="small"
            @click="deleteClick(scope.row.titleId)"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        titleName: [{ required: true, message: "请填写题目", trigger: "blur" }],
        titleType: [
        ...
        choice1: [{ required: true, message: "不能为空", trigger: "blur" }],
        choice2: [{ required: true, message: "不能为空", trigger: "blur" }],
        choice3: [{ required: true, message: "不能为空", trigger: "blur" }],
        choice4: [{ required: true, message: "不能为空", trigger: "blur" }],
      },
    };
  },
  watch: {
  ...
  methods: {
    ...
    updateQuestions() {
      ...
      this.axios
        .post("/wsyu/title/updateTitle.htm", params)
        .then(
          function (response) {
    ...
    insertQuestions() {
      ...
      this.axios
        .post("/wsyu/title/insertByTitle.htm", params)
        .then(
          function (response) {
            if (response.data.code == "0000") {
    ...
    querySubjectList() {
      this.axios
        .post("/wsyu/sub/queryListBySub.htm")
        .then(
          function (response) {
    ...
    handleClick(row) {
      ...
      this.axios
        .post("/wsyu/title/queryTitleInfo.htm", params)
        .then(
          function (response) {
    ...
    deleteClick(row) {
      ...
      this.axios
        .post("/wsyu/title/deleteTileInfo.htm", params)
        .then(
          function (response) {
            if (response.data.code == "0000") {
    ...
    queryPage() {
      ...
      this.axios
        .post("/wsyu/title/queryTitlePage.htm", params)
        .then(
          function (response) {
